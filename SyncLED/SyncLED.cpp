...
void SyncLED::begin(byte pin, byte initialState, boolean usePWM, unsigned long pwmMS)
{
	m_pin = pin;
	m_state = initialState;
	m_inPattern = false;
	m_lastChange = 0L;
	m_usePWM = usePWM && digitalPinHasPWM(pin);
	m_pwmFadeMS = pwmMS;
	m_duration = 0;
...
void SyncLED::On(unsigned long duration)
{
	m_state = HIGH;
...
void SyncLED::update(unsigned long time_ms)
{
	if (!m_inPattern)
	{
		if (isOn() && (m_duration > 0) && (time_ms > m_lastChange + m_duration))
			Off();
		return;
	}
